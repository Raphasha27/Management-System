      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <div className={styles.info}>
            ...
            <p>Track progress and milestones of ongoing IT projects.</p>
          </div>
            ...
            <div key={project.id} className={styles.projectCard}>
              <div className={styles.projectMain}>
                <div className={styles.projectTitle}>
                  <div className={styles.iconBox} style={{ background: '#2563eb15', color: '#2563eb' }}>
                    <Code size={20} />
                  </div>
                  <div>
                    <h3>{project.name}</h3>
                    <div className={styles.clientInfo}>
                      <User size={14} />
                      <span>{project.client.name} | {project.client.company || 'Private'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.statusBadge}>
                  ...
                  <span className={`${styles.statusText} ${styles[project.status.toLowerCase().replace(' ', '')]}`}>
                    {project.status}
                  </span>
                </div>
              </div>
                ...
                <div className={styles.progress}>
                  <div className={styles.progressBar}>
                    <div 
                        ...
                        style={{ width: project.status === 'Completed' ? '100%' : project.status === 'Active' ? '65%' : '0%' }}
                    ></div>
                  </div>
                  <div className={styles.progressText}>
                    {project.status === 'Completed' ? '100%' : project.status === 'Active' ? '65%' : 'Pending'}
                  </div>
                </div>
              </div>
            </div>
